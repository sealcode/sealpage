...
class S {
	...
	async addOutputFile({ output_subdir, base_name, generator, deps }) {
		...
		try {
			await fileExists(path.resolve(output_path, '..'));
		} catch (error) {
	...
	require(module_name) {
		try {
			...
			console.error(e);
			throw new Error(
				`Could not resolve module '${module_name}'. It might not be installed or you're running the s.require function in the browser, while it's only supposed to be called on backend side`
			);
		}
	}
}
